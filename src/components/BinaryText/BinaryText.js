...
class BinaryText extends PureComponent {
  ...
  initializeScene() {
    ...
    this.scene = new ScrollMagic.Scene({ offset: 0, triggerHook: 0.9 })
  ...
  renderWrappedWord(word, indexOuter) {
    ...
    const wrappedLetters = word.map((letter, indexInner) =>
      <BinaryCharacter key={indexInner} text={letter} isInverted={isInverted} />
    );

    return (
      <div key={indexOuter} styleName="binary-word">
        {wrappedLetters}
      </div>
    );
  }
  ...
  render() {
    return (
      <div ref={this.setTriggerElementRef.bind(this)}>
        <div styleName="binary-text" ref={this.setTargetElementRef.bind(this)}>
          {this.renderIfValid()}
        </div>
      </div>
    );
