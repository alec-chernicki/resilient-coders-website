...
class UIHero extends React.Component {
  componentDidMount() {
    const tweenOne = TweenMax.from('.animated-character', 0.65, {
      opacity: 0,
      x: -30,
      ease: Linear.easeNone,
    });
    const tweenTwo = TweenMax.from('.hero-divider', 0.25, {
      width: 0,
      ease: Linear.easeNone,
    });
    const tweenThree = TweenMax.from('.hero-text', 0.25, {
      opacity: 0,
      y: 20,
      ease: Linear.easeNone,
    });
    const tweenFour = TweenMax.from('.hero-button', 0.25, {
      opacity: 0,
      y: 20,
      ease: Linear.easeNone,
    });

    const heroContentTimeline = new TimelineMax({ paused: true }).add(
      [tweenOne, tweenTwo, tweenThree, tweenFour],
      0.05,
      'sequence',
      0
    );
  ...
  getWrappedWords(text) {
    ...
    return words.map((word, key) => {
      return (
        <span key={key} styleName="title-word">
  ...
  renderBackground() {
    ...
    if (video) {
      return <UIVideoLayer src={video} className="index-1" />;
    }

    if (image) {
      return <UILayer image={image} className="index-1" />;
    }
  }
  renderTitleOne() {
